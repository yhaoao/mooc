    ...
    def update(self):
        ...
        for item in self.viruses:
            if item.doesClear():
                ...
                try:
                    newViruse=item.reproduce(popDensity)
                    self.viruses.append(newViruse)
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for i in range(numTrials):
        ...
        for j in range(numViruses):
            viruse=SimpleVirus(maxBirthProb, clearProb)
    ...
    for k in range(300):
        ...
        for n in range(len(patients)):
            allSize.append(patients[n].update())
        sizes.append(sum(allSize)/float(len(allSize)))
    pylab.plot(range(300), sizes)
    ...
    def update(self):
        ...
        for item in self.viruses:
            if not item.doesClear():
                try:
                    viruses.append(item)
                    newViruse=item.reproduce(popDensity,self.prescriptions)
                    viruses.append(newViruse)
                except NoChildException:
                    pass
        self.viruses=viruses
        return len(self.viruses)
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    pylab.plot(range(300), sizes)
    pylab.plot(range(300), resistSizes)
    pylab.title('SimpleVirus simulation')
    pylab.xlabel('Time Steps')
    pylab.ylabel('Average Virus Population')
    pylab.legend('SimpleVirus')
